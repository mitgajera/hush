...
// Runtime check: is the adapter a wallet-standard StandardWalletAdapter?
function getStdWalletAndAccount(adapter: unknown): {
  ...
  if (
    ...
    if (Array.isArray(a.wallet?.accounts) && a.wallet.accounts.length > 0) {
      return {
        wallet: a.wallet as object,
        account: a.wallet.accounts[0] as object,
      }
    }
  }
  return null
...
export function useUmbra() {
  ...
  useEffect(() => {
    ...
    // Cast to the wallet-standard types the Umbra SDK expects
